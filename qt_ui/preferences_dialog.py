...
class PreferencesDialog(QDialog, Ui_PreferencesDialog):
    ...
    def loadSettings(self):
        ...
        if hasattr(self, 'icon_theme_combobox'):
            ...
            if index >= 0:
                blocked = self.icon_theme_combobox.blockSignals(True)
                self.icon_theme_combobox.setCurrentIndex(index)
                self.icon_theme_combobox.blockSignals(blocked)
        
        # dark mode
        if hasattr(self, 'dark_mode_checkbox'):
            blocked = self.dark_mode_checkbox.blockSignals(True)
            self.dark_mode_checkbox.setChecked(qt_ui.settings.dark_mode_enabled.get())
            self.dark_mode_checkbox.blockSignals(blocked)
    ...
    def saveSettings(self):
        ...
        # icon theme
        if hasattr(self, 'icon_theme_combobox'):
            selected_theme = self.icon_theme_combobox.currentData() or self.icon_theme_combobox.currentText()
            if selected_theme:
                qt_ui.settings.icon_theme.set(str(selected_theme))
                    ...
                    for icon_name in icon_files:
                        ...
                        if os.path.exists(icon_path):
                            try:
                                ...
                                self.icon_theme_combobox.addItem(icon, icon_name, icon_name)
                            except Exception as e:
                                logger.warning(f"Failed to load icon {icon_name}: {e}")
                                self.icon_theme_combobox.addItem(icon_name, icon_name)
                        else:
                            self.icon_theme_combobox.addItem(icon_name, icon_name)
                else:
                    # If resources dir doesn't exist, just add hardcoded options
                    for theme_name in ['cherries', 'banana', 'walnut', 'coyote']:
                        self.icon_theme_combobox.addItem(theme_name, theme_name)
            except Exception as e:
